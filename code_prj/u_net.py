    ...
    def forward(self, x):
        ...
        for layer in self.enc_layers:
            if isinstance(layer, CNNBlocks):
                # print(layer)
                # print('befor CNNBlocks', x.shape)
                x = layer(x)
                # print('past CNNBlocks', x.shape)
                # print('')
                route_connection.append(x)
            else:
                # print(layer)
                # print('befor Maxpool', x.shape)
                x = layer(x)
                # print('past Maxpool', x.shape)
                # print('')
...
class UNET(nn.Module):
    def __init__(
        self, in_channels, first_out_channels, exit_channels, downhill, padding=0, use_cfr=False
    ):
        ...
        if self.use_cfr:
            self.cfr = CRF(n_spatial_dims=exit_channels)
